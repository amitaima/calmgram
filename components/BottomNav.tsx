...
export const BottomNav: React.FC<BottomNavProps> = ({ currentUserAvatar, currentView, onViewChange }) => {
  const getIconClass = (viewName: string) => {
    ...
    return `p-3 transition-colors duration-300 ${isActive ? 'text-stone-800 scale-105' : 'text-calm-subtext hover:text-calm-text'}`;
  };

  return (
    <div className="h-16 w-full border-t border-stone-100 flex justify-around items-center px-2 pb-1 bg-calm-bg/95 backdrop-blur-md">
      <button 
      ...
      >
        <div className={`p-0.5 rounded-full ${currentView === 'profile' ? 'border-2 border-stone-800' : 'border border-stone-300'}`}>
          <img 
